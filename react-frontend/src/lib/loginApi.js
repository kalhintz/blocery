...
export const doProducerLogout = () => axios(Server.getRestAPIHost() + '/producerLogin', { method: "delete", withCredentials: true, credentials: 'same-origin' })
    .then((response) => {
        //autoLogin false,
        console.log('do Producer Logout');
...
export const doKakaoLogin = (data) => axios(Server.getRestAPIHost() + '/kakaoLogin', {method: "post", params:{ accessKey: data }, withCredentials: true, credentials: 'same-origin'})

// kakao 소비자 6자리 비번 등록 프로세스
export const doKakaoConsumer = ({consumerNo,passPhrase}) => axios(Server.getRestAPIHost() + '/kakaoConsumer', {method: "post", params:{ consumerNo: consumerNo, passPhrase: passPhrase }, withCredentials: true, credentials: 'same-origin'})
