...
import { getSeller, getStaticsGiganSalesListBySellerNo } from '~/lib/b2bSellerApi'
...
import Order from '~/components/b2bSeller/deal'
...
export default class GiganSalesSttList extends Component {
    ...
    getColumnDefs () {
        ...
        columnDefs = [
            ...
            {
                headerName: "할인비", field: "totalDiscountAmt",
                width: 150,
                cellStyle:this.getCellStyle({cellAlign: 'right'}),
                cellRenderer: 'formatCurrencyRenderer',
                filterParams: {
    ...
    */
    ...
    search = async () => {
        ...
        const { status, data } = await getStaticsGiganSalesListBySellerNo(dataParams);
        //console.log(status);
        //console.log(data);
        if(status !== 200){
            alert('응답이 실패 하였습니다');
