...
import { getSeller, updateSellerPush } from '~/lib/b2bSellerApi'
...
export default class Setting extends Component {
    ...
    async componentDidMount() {
        const loginUser = await getSeller();
    ...
    changeTrue = async () => {
        let data = {};
        data.sellerNo = this.state.loginUser.sellerNo;
        data.name = this.state.loginUser.name;
        data.receivePush = true;

        const modified = await updateSellerPush(data);
    ...
    render() {
        return(
            <Fragment>
                <B2bShopXButtonNav history={this.props.history}>설정</B2bShopXButtonNav>
                <br/>
