...
import { getConfirmedDealBySellerNo } from "~/lib/b2bSellerApi"
import { getBuyerFoodsBySellerNo } from "~/lib/b2bFoodsApi"
...
export default class BlctHistory extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            isOpen: false,
            copied: false,
            dealInfoList: [],       // 사용내역 조회시 필요한 주문정보
            foodsInfoList: [],        // 사용내역 조회시 필요한 상품정보
            blctToWon: ''           // BLCT 환율
        }
    }
    ...
    getUser = async () => {
        ...
        let loginUser = await getB2bLoginUser();
    ...
    search = async () => {
        ...
        // 로그인한 생산자의 주문목록 조회
        const { data:dealList } = await getConfirmedDealBySellerNo();
        const { data:foodsList } = await getBuyerFoodsBySellerNo(this.state.loginUser.uniqueNo)  // 로그인된 생산자가 등록한 상품목록 조회

        // 주문목록에서 필요한 필드 뽑아서 dealInfoList에 저장
        let dealsResults = dealList.map((row,i) => {
            return {dealSeq: row.dealSeq, foodsNo: row.foodsNo, orderDate: row.orderDate, goodsNm: row.goodsNm, consumerNm: row.consumerNm, consumerEmail: row.consumerEmail};
        })
        let foodsResults = foodsList.map((row,i) => {
            return {foodsNo: row.foodsNo, goodsNm: row.goodsNm, timestamp: row.timestamp}
        })

        this.setState({
            dealInfoList: dealsResults,
            foodsInfoList: foodsResults,
            blctToWon: blctToWon
        })
    ...
    getBlctHistory = async () => {
        const deals = this.state.dealInfoList
        const foods = this.state.foodsInfoList
        const list = []

        const blctListDeal = deals.map(async deal => {
            const { data: result } = await scOntGetProducerOrderBlctHistory(deal.dealSeq)
            ...
            if(result.receiveOrderRewardBlct > 0){
                ...
                if(date) {
                    list.push({
                        ...
                        date: date,
                        contents: deal.goodsNm + ' | 고객명:' + deal.consumerNm + '(' + deal.consumerEmail + ')',
        ...
        const blctListFoods = foods.map(async food=>{
            const { data:result2 } = await scOntGetProducerGoodsBlctHistory(food.foodsNo)

            // payFoodsDeposit(-판매등록시 거는 미배송 보상금)
            if(result2.payFoodsDeposit > 0){
                const date = food.timestamp
                if(date) {
                    list.push({
                        blct: '- '+result2.payGoodsDeposit,
                        date: date,
                        contents: food.goodsNm + ' 판매등록',
        ...
        await Promise.all(blctListDeal)
