...
import { getOperStatDealCntBySellerNo } from '~/lib/b2bSellerApi'
...
const DealDeal = (props) => {
    ...
    async function search() {
        ...
        // 주문건수 (오늘, 어제, 주간, 월간)
        const { status, data } = await getOperStatDealCntBySellerNo();
        //console.log("orderdeal_status===",status)
        //console.log("orderdeal===",data)
        if(status === 200) {
        ...
        <Fragment>
            ...
            <div className='mr-1'>
                <div className='p-2 bg-secondary text-white'>
                    <div className='text-center f6 font-weight-light'>주문건수</div>
