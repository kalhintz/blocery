...
import { getSeller, getFoodsQnaListBySellerNo } from '~/lib/b2bSellerApi'
...
import Style from './FoodsQnaList.module.scss'
//ag-grid
import { AgGridReact } from 'ag-grid-react';
import 'ag-grid-community/dist/styles/ag-grid.css';
import 'ag-grid-community/dist/styles/ag-theme-balham.css';
import FoodsQnaAnswer from "./FoodsQnaAnswer";
...
export default class FoodsQnaList extends Component {
    ...
    goodsQnaMobileRenderer = ({value, data:rowData}) => {
        let goodsQueDate = rowData.goodsQueDate ? ComUtil.utcToString(rowData.goodsQueDate):null;
        return (
            <Cell height={this.rowHeight}>
                <div>
                    <span className='small'>문의번호 : {rowData.foodsQnaNo}</span><br/>
                    <span className='small'>상품명 : ({rowData.foodsNo}) {rowData.goodsName}</span><br/>
                    <span className='small'>작성일 : {goodsQueDate}</span><br/>
    ...
    goodsQnaStatRenderer = ({value, data:rowData}) => {
        let txtColor = rowData.goodsQnaStat === 'ready' ? 'text-primary':null;
            ...
            <Cell height={this.rowHeight}>
                <div style={{textAlign:'center'}}>
                    ...
                    <Button size={'sm'} className={'bg-primary'}
                            onClick={this.openAnswerPopup.bind(this, rowData)}>{ goodsQnaStatBtnVal }</Button>
                </div>
            </Cell>
        );
    }
    ...
    */
    ...
    search = async () => {
        ...
        const { status, data } = await getFoodsQnaListBySellerNo();
        if(status !== 200){
            alert('응답이 실패 하였습니다');
            return
        }
    ...
    openAnswerPopup = (goodsQna) => {
        ...
        this.setState({
            foodsQnaNo: goodsQna.foodsQnaNo
        })
    ...
    render() {
        return(
            <div>
                ...
                <ModalWithNav show={this.state.isAnswerModalOpen} title={this.titleOpenAnswerPopup} onClose={this.onAnswerPopupClose}>
                    <div className='p-1' style={{width: '100%',minHeight: '360px'}}>
                        <FoodsQnaAnswer foodsQnaNo={this.state.foodsQnaNo} onClose={this.onAnswerPopupClose} />
                    </div>
                </ModalWithNav>
            </div>
        );
