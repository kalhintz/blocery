...
import { faCartPlus, faClock, faBolt, faSpinner, faExclamationCircle } from '@fortawesome/free-solid-svg-icons'
...
import { getFarmDiaryBykeys, getGoodsReviewByGoodsNo, getGoodsQnAByKeys, getOtherGoodsReviewByItemNo, isBlyTimeBadge, getGoodsBannerList } from '~/lib/shopApi'
import { getGoodsContent, getBlyReview } from '~/lib/goodsApi'
...
import { exchangeWon2BLCTHome, exchangeWon2BLCTPoint, exchangeWon2BlctBlyTime, blctToWonWithTime } from "~/lib/exchangeApi"
        ...
        <Fragment>
            ...
            <div className={Css.wrap}>
                    ...
                    </Div>
                    {
                        blyTime && inBlyTimePeriod &&
                            <Div>
                                ...
                                <BlyTime fontSize={11} bg={'danger'} fg={'white'} ml={5} mr={4}>블리타임</BlyTime>
                        ...
                        <div className={Css.left}>
                            {
                                discountRate > 0 && (
                                    <div className={Css.discountBox}>
                                        ...
                                        <div className={Css.consumerPrice}><strike>{ComUtil.addCommas(consumerPrice)}원</strike></div>
                                    </div>
                                )
                            }
                ...
                <div className={Css.shippingInfoBox}>
                    {
                        directGoods ? (
                            <div>
                                ...
                                <div className={Css.dark}>지금 구매하면 2~3일 이내로 받아보실 수 있습니다.</div>
                            </div>
                        ) : (
                            <div>
                                <div>
                                ...
                                <div className={Css.dark}>지금 구매하면 <span className={Css.black}>{ComUtil.utcToString(expectShippingStart, 'MM[/]DD')}~{ComUtil.utcToString(expectShippingEnd, 'MM[/]DD')}</span> 사이에 발송되며, 발송 후 2~3일 이내에 받아보실 수 있습니다.</div>
                            </div>
                        )
                    }
...

                    {
                        (packCnt - remainedCnt) >= 50 && (
                            <>
                            <div>
                                <span>수량 </span><span className={Css.black}><b>구매건수 {ComUtil.addCommas(packCnt - remainedCnt)} / 잔여 {ComUtil.addCommas(remainedCnt)}</b></span>
                            </div>
                            <hr className={Css.lineLight} style={{margin: '0 16px'}}/>
                            </>
                        )
...

                {
                    //상품정보
                    (tabId === 1) && (
                        goodsBannerList.length > 0 ? (
                            <div>
                                {
                                    goodsBannerImages.map((image)=>{
                                        return (
                                            <div>
                                                <img style={{width:'100%',height:'500'}} src={serverImageUrl + image} alt="공지배너"/>
                                                {
                                                    !goodsContent ? (
