import React, {Fragment, useState} from 'react'
import PropTypes from 'prop-types'
import { B2bShopXButtonNav, QtyInputGroup } from '../../common'
import { InputGroup, InputGroupAddon, InputGroupText, Input, Button, Label } from 'reactstrap';
import { Checkbox } from '@material-ui/core'
import { ArrowBack, Close, CloseSharp } from '@material-ui/icons'
import { Add, Remove } from '@material-ui/icons'
import ComUtil from '../../../util/ComUtil'
import { Server } from '../../Properties'
import Style from './CartList.module.scss'

import { TERMS_OF_DELIVERYFEE } from '~/lib/bloceryConst'

const FoodsItem = (props) => {

    //수량 변경
    const onQtyChange = (res) => {
        props.onChange({
            type: 'UPDATE_QTY',
            state: {
                name: props.name,
                sellerNo: props.sellerNo,
                foodsNo: props.foodsNo,
                qty: res.value
            }
        })
    }

    //체크박스 변경
    const onCheckboxChange = (e) => {
        props.onChange({
            type: 'UPDATE_CHECKED',
            state: {
                name: props.name,
                sellerNo: props.sellerNo,
                foodsNo: props.foodsNo,
                checked: e.target.checked
            }
        })
    }

    //삭제 클릭
    const onDeleteClick = () => {
        props.onChange({
            type: 'DELETE',
            state: {
                name: props.name,
                sellerNo: props.sellerNo,
                foodsNo: props.foodsNo
            }
        })

    }

    const totPrice = props.goodsPrice + props.deliveryFee

    //상품클릭
    function onClick(){
        props.history.push(`/b2b/foods?foodsNo=${props.foodsNo}`)
    }

    const checkBoxId = 'check_' + props.foodsNo

    return (
        <Fragment>

            {/* 제품 박스 start */}
            <div className='bg-white p-2 pl-3 pr-3'>
                {/* 제품명 박스 start */}
                <div className='d-flex align-items-center mb-2'>
                    <Checkbox id={checkBoxId} className={'p-0'} color={'default'} checked={props.checked} onChange={onCheckboxChange} />
                    <Label for={checkBoxId} className='m-0 f5'>{props.goodsNm}</Label>
                    <div className='ml-auto'><span onClick={onDeleteClick}><CloseSharp /></span></div>
                </div>
                {/* 제품명 박스 end */}

                {/* 이미지 & 수량 박스 start */}
                <div className='d-flex align-items-top mb-2'>

                    {/* 이미지 */}
                    <div>
                        {/*<img className={Style.goodsImage} src={'data:image/jpeg;base64,'} />*/}
                        <img className={Style.goodsImage} src={Server.getThumbnailURL()+props.goodsImages[0].imageUrl} onClick={onClick} alt={'사진'}/>
                        <div className='text-secondary small text-center p-1'>
                            {ComUtil.addCommas(props.currentPrice)} 원 ({Math.round(props.discountRate)}%)
                        </div>
                    </div>

                    {/* 수량 & 합계 */}
                    <div className='flex-grow-1 ml-2'>

                        {/* 수량 */}
                        <div className='d-flex justify-content-end align-items-end mb-2'>
                            <QtyInputGroup readOnly onChange={onQtyChange} name={props.foodsNo} value={props.qty}/>
                        </div>




                        <div className='d-flex mb-1 text-dark f13'>
                            <div className='text-right ml-auto'>상품가</div>
                            <div className='text-right' style={{minWidth: 100}}>{ComUtil.addCommas(props.currentPrice)} 원</div>
                        </div>
                        {
                            //택배 일 경우만 배송비를 보여준다
                            props.taekbaeDelivery && (
                                <div className='d-flex mb-1 text-dark f13'>
                                    <div className='text-right ml-auto'>배송비</div>
                                    <div className='text-right' style={{minWidth: 100}}>+ {ComUtil.addCommas(props.calculatedDeliveryFee)} 원</div>
                                </div>
                            )
                        }
                        <div className='d-flex font-weight-border f13'>
                            <div className='text-right ml-auto'>합계</div>
                            <div className='text-right' style={{minWidth: 100}}>{ComUtil.addCommas(totPrice)} 원</div>
                        </div>


                    </div>

                </div>
                {/* 재고부족 메시지 */}
                {
                    props.qty > props.remainedCnt && <div className='text-center text-danger f7'>재고부족 상품입니다(재고{ComUtil.addCommas(props.remainedCnt)}건)</div>
                }
                {/* 이미지 & 수량 박스 end */}
                {
                    //몇개당 배송비 부과상품은 무료배송 미지원 상품
                    props.termsOfDeliveryFee === TERMS_OF_DELIVERYFEE.EACH_GROUP_COUNT && <div className='text-center text-primary f7'>배송비할인 제외 상품입니다</div>
                }

            </div>
            {/* 제품 박스 end */}
        </Fragment>
    )
}

FoodsItem.propTypes = {
    name: PropTypes.string.isRequired,
    foodsNo: PropTypes.number.isRequired,
    checked: PropTypes.bool,
    goodsPrice: PropTypes.number.isRequired,
    deliveryFee: PropTypes.number.isRequired,
}
FoodsItem.defaultProps = {
    name: '',
    checked: true,
    goodsPrice: 0,
    deliveryFee: 0,
}


export default FoodsItem