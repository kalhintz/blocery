...
const ItemKind = (props) => {
    ...
    return(
        <Container fluid>
            <Row>
                {
                    props.item && <Item itemNo={props.item.itemNo} itemKindName={'전체선택'} itemKindCode={'all'} isActive={itemKindCode === 'all'} onClick={onClick.bind(this, 'all')} />
                }
                {
                    props.item && props.item.itemKinds.map((itemKind, index)=>
                        <Item key={'category_itemKind'+index} itemNo={props.item.itemNo} itemKindName={itemKind.name} itemKindCode={itemKind.code} isActive={itemKindCode == itemKind.code} onClick={onClick.bind(this, itemKind.code)} />
                    )
                }
            </Row>
        </Container>
