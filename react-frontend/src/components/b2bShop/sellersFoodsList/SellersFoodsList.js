...
import { B2bShopXButtonNav, B2bSlideItemHeaderImage, B2bSlideItemContent, HeaderTitle, ModalWithNav } from '~/components/common'
import { getBuyerFoodsBySellerNoSorted, getBuyerFoodsBySellerNoAndItemNoSorted } from '~/lib/b2bFoodsApi'
...
const sorters = [
    ...
    // {value: 2, label: '인기'},
    {value: 3, label: '마감임박', sorter: {direction: 'DESC', property: 'saleEnd'}},
...
const SellersFoodsList = (props) => {
    ...
    async function search(filter, sorter) {
        ...
        if(sellerNo){
            ...
            if(filter.value === -1){
                const {data} = await getBuyerFoodsBySellerNoSorted(sellerNo, sorter)
                setFoodsList(data)
            }
            //필터의 품목이 있을 경우 품목, 정렬 둘다 적용하여 상품조회
            else{
                const {data} = await getBuyerFoodsBySellerNoAndItemNoSorted(sellerNo, filter.value, sorter)
                setFoodsList(data)
            }
        }
    ...
    function movePage(foodsNo) {
        props.history.push(`/b2b/foods?foodsNo=${foodsNo}`)
        // Webview.closePopupAndMovePage(`/b2b/foods?foodsNo=${foodsNo}`)
    }
    ...
    return(
        <div>
            ...
            <B2bShopXButtonNav fixed
                               history={props.history} back>판매상품</B2bShopXButtonNav>
            <HeaderTitle
                sectionLeft={<div>총 {ComUtil.addCommas(foodsList.length)}개 상품</div>}
                sectionRight={
                    <Fragment>
                        <ModalCheckListGroup
                            title={'필터 설정'}
                        ...
                        />
                        <ViewButton icons={[<ViewModule />, <ViewStream />]} onChange={onViewChange} />
                    </Fragment>
                }
            />
                            ...
                            <Row>
                                {
                                    foodsList.map(foods =>
                                        <Col
                                            key={'foods'+foods.foodsNo}
                                            xs={viewIndex === 0 ? 6 : 12}
                                            sm={viewIndex === 0 ? 4 : 12}
                                            lg={viewIndex === 0 ? 3 : 12}
                                            xl={viewIndex === 0 ? 2 : 12}
                                            className='p-0'
                                            onClick={movePage.bind(this, foods.foodsNo)}
                                        >
                                            <div className='mr-2 mb-2 border'
                                                // onClick={movePage.bind(this, {type: 'FOODS_DETAIL', payload: {foodsNo: foods.foodsNo}})}
                                            >
...
export default SellersFoodsList