...
import {getDonTotal, donWithdrawRequest, withdrawDonStatus} from "~/lib/donAirDropApi";
...
class WithdrawErcDon extends Component {
    constructor(props){
        super(props)
        this.state = {
    ...
    modalToggleOk = async () => {
        ...
        //신규: 출금요청
        let {data:result} = await donWithdrawRequest(this.state.ercAccount);
    ...
    render() {
        return (
            <Fragment>
                ...
                {/*}*/}
                <ShopXButtonNav underline historyBack>ERC 출금</ShopXButtonNav>
                {
                ...
                <Div p={16} bg={'backgroundDark'}>
                    ...
                    <Card shadow={'sm'}>
                        <HeadingLayout>
                            <Div fontSize={16} fw={500}>지갑주소 <Span fg={'danger'}>ERC</Span></Div>
                    ...
                    <Card shadow={'sm'} fontSize={13}>
                        ...
                        <Button  mt={16} fontSize={16} bg={'green'} fg={'white'} rounded={2} block py={12} disabled={!this.state.withdrawAmount || !this.state.accountOk} onClick={this.onWithdrawBlct} >ERC 출금신청하기</Button>
                    </Card>
                    <Card shadow={'sm'}>
...
export default withRouter(WithdrawErcDon)